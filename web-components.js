class TalkRecorderApp extends HTMLElement {
    ...
    render() {
        this.shadowRoot.innerHTML = `
            ...
            <div class="chat-window">
                ...
                <div class="controls">
                    <button class="start-record-btn">Iniciar Grabación</button>
                    <button class="stop-record-btn" disabled>Detener Grabación</button>
                    <span class="recording-indicator">Grabando...</span>
                    <button class="copy-btn">Copiar</button>
                    <button class="export-pdf-btn">Exportar a PDF</button>
    ...
    async grantPermissions() {
        const ownerInput = this.shadowRoot.getElementById("owner-name").value.trim();
        if (!ownerInput) {
        ...
        if ("showDirectoryPicker" in window) {
            try {
                ...
                this.shadowRoot.getElementById("permissions-modal").style.display = "none";
                this.setupSpeechRecognition();
                this.setupMediaRecorder();
            } catch (err) {
            ...
            alert("Este navegador no soporta acceso a carpetas. Los archivos se descargarán manualmente.");
            this.shadowRoot.getElementById("permissions-modal").style.display = "none";
            this.setupSpeechRecognition();
            this.setupMediaRecorder();
        }
    ...
    async revalidatePermissions() {
        if ("showDirectoryPicker" in window) {
            try {
                const permissionStatus = await this.directoryHandle.queryPermission({ mode: "readwrite" });
    ...
    setupSpeechRecognition() {
        ...
        recognition.onresult = (event) => {
            ...
            this.chats.push({ text: transcript, timestamp: new Date().toISOString() });
            this.saveDataToJson();
        };
        recognition.onerror = (event) => console.error("Error en Speech Recognition:", event.error);
    ...
    setupEventListeners() {
        this.shadowRoot.addEventListener("click", (e) => {
            ...
            if (e.target.matches(".start-record-btn")) {
                ...
                navigator.clipboard.writeText(chatText.value).then(() => console.log("Texto copiado"));
        ...
        this.shadowRoot.querySelector("#load-recording").addEventListener("change", (e) => {
    ...
    startRecording() {
        ...
        this.shadowRoot.querySelector(".recording-indicator").classList.add("active");
    }
    ...
    stopRecording() {
        ...
        this.shadowRoot.querySelector(".recording-indicator").classList.remove("active");
    }
    ...
    async saveDataToJson() {
        ...
        const fileHandle = await this.directoryHandle.getFileHandle("talkrecorder_data.json", { create: true });
        const writable = await fileHandle.createWritable();
    ...
    async loadDataFromJson() {
        if (!this.directoryHandle) return;
        try {
            const fileHandle = await this.directoryHandle.getFileHandle("talkrecorder_data.json");
            ...
            console.log("No se encontró un archivo JSON previo o está corrupto. Se creará uno nuevo.");
        }
    }
        ...
        if (this.directoryHandle && "values" in this.directoryHandle) {
            for await (const entry of this.directoryHandle.values()) {
                if (entry.kind === "file" && entry.name.endsWith(".webm")) {
                    ...
                    html += `
                        <li>
                            ${entry.name} (${new Date(file.lastModified).toISOString()})
                            <button class="play-recording-direct" data-file-name="${entry.name}">Reproducir</button>
                        </li>
                    `;
                }
            }
        } else {
            html = this.recordings.map((rec) => `
                <li>
